package ppf.tool.components.mx.components.controls
{
	...
	public class DateChooser extends mx.controls.DateChooser
	{
		 ...
		 * 是否可编辑时、分、秒
		 */ 
		//[Inspectable(type="Boolean", enumeration="true,false" , defaultValue="true")] 
		public var _isEditHmsAble:Boolean = true;
		
		public function set isEditHmsAble(val:Boolean):void{
			_isEditHmsAble = val;
			if(_hourInput)
				_hourInput.enabled = val;
			if(_minuteInput)
				_minuteInput.enabled = val;
			if(_secondInput)
				_secondInput.enabled = val;
		}
		
		public function get isEditHmsAble():Boolean{
			return _isEditHmsAble;
		}
