package ppf.tool.components.mx.components.controls
{
  ...
  [IconFile("/assets/ScrollableMenu.png")]
  ...
  public class ScrollableMenu extends Menu
  {
    ...
    {
		super();
		this.addEventListener(Event.RENDER,__xFunc,false,0,true)
...
//	  }
	...
	//  尽量避免菜单折叠
	protected  function __xFunc(e:Event):void
	{
		
  		if(this.visible&&(this.y+this.height>this.stage.stageHeight-40))
		{
			if(this.stage.stageHeight-40-this.height<0)
			{
				this.y=0
			}else
			{
				this.y=this.stage.stageHeight-40-this.height
			}
		}
	}
	 
	
	protected function  _removeXfunc(e:Event):void
	{
		this.removeEventListener(FlexEvent.SHOW,__xFunc)
		this.removeEventListener(FlexEvent.HIDE,_removeXfunc)
	}
