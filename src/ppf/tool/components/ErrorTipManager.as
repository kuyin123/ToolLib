package ppf.tool.components
{
	...
	/**
	 * 始终显示Validator类的错误提示，而不是通过鼠标移动到控件上面才显示
	 * showErrorTip()
	 * hideErrorTip()
	 * @author wangke
	 */
	public class ErrorTipManager
	{
		 ...
		 * 
		 */		
		public static function unregisterValidator(validator:Validator,clearErrorString:Boolean=false):void
		{
			...
			removeErrorTip(validator.source,clearErrorString);
		...
		public static function registerValidatorOnPopUp(validator:Validator, popUp:UIComponent, hideExistingErrorTips:Boolean=false):void
		{
			...
			if (popUps[popUp] == null)
			{
				...
				popUp.addEventListener(Event.REMOVED_FROM_STAGE,onRemoveFromStage,false,0,true);
			}
		 ...
		 */
		public static function unregisterPopUpValidators(popUp:UIComponent,validateExistingErrorTips:Boolean=false,clearErrorString:Boolean=false):void
		{
			if (popUps[popUp] != null)
			{
				var validators:Array=(popUps[popUp] as Array);
				...
				{
					unregisterValidator(validator,clearErrorString);
				}
				delete popUps[popUp];
				...
				popUp.removeEventListener(Event.REMOVED_FROM_STAGE, onRemoveFromStage);
			}
					...
					var id:uint=setTimeout(function():void
						{
							...
							for each (var target:DisplayObject in targets)
							{
								...
								if ((pt.x < 0) || (pt.y < 0) || ((pt.x + target.width) > parent.width) || ((pt.y + target.height) > parent.height))
								{
		...
		private static function sizeErrorTip(errorTip:IToolTip):void
		{
			...
			errorTip.setActualSize(errorTip.getExplicitOrMeasuredWidth(), errorTip.getExplicitOrMeasuredHeight());
		}

		/**
		 * 显示错误的提示，如果该目标对象没有则创建
		 * @param target 目标对象
		 * @param error 错误的提示字符
		 * 
		 */		
